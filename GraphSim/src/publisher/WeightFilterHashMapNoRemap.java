...
public class WeightFilterHashMapNoRemap {
	...
	public void recursiveUnionfind(GraphNoRemap g, int depth, int sizeLimit, double weightThreshold) throws IOException{
		...
		for(Set <Integer> cur : nextStep){
			GraphNoRemap subG = g.getSubGraph(cur);
...
//		wfhm.writeComponents(args[1]);
		...
		WeightFilterHashMapNoRemap wfhm = new WeightFilterHashMapNoRemap();
		GraphNoRemap g = new GraphNoRemap(args[0], 1,  ",",  4.58, 1000, 1, 2, 6);
		Map <Integer, Set <Integer>> components = g.getComponents();
		...
		for(Set <Integer> component: bigComponents){
			GraphNoRemap subG = g.getSubGraph(component);
